...
// Battery saving: the radio is only useful while the user is actually pairing
// with the app. If nobody connects within this window the BLE controller is
// powered down completely for the rest of this power cycle. Playback via the
// main button and the (separate RF) remote are unaffected.
#define BLE_IDLE_TIMEOUT_MS 120000UL  // 2 minutes

// Flatch product-family service UUID (must match the app's FlatchBle.serviceUuid).
// UUID migration APPROVED BUT DEFERRED: at the next reflash, change this AND the
// app constant together to "b88599a1-30f2-4618-b1f1-a8a5a2d10a60".
...
// Start of the current advertising window (set at boot and again on every
// disconnect). Unsigned subtraction against millis() so the 49-day rollover is
// handled correctly.
static uint32_t g_bleWindowStartMs = 0;
// Once true the BLE controller has been torn down and CANNOT be brought back
// without a reboot — by design, the user power-cycles the Flatch to pair.
static bool g_bleShutdown = false;
...
static void restartAdvertisingSafe() {
  if (g_bleShutdown) return;  // radio is gone; nothing to advertise with
...
// Powers the Bluetooth controller down for the remainder of this power cycle.
//
// BLEDevice::deinit(true) disables + deinitialises bluedroid AND the controller
// and then releases the BT memory pool, which is the deepest saving available
// without sleeping the whole chip. Releasing that pool is irreversible until a
// reboot, which is exactly the intended behaviour: to pair, the user turns the
// Flatch off and on again.
//
// Only ever called with no active connection, so no callback can be mid-flight.
static void shutdownBle() {
  if (g_bleShutdown) return;
  g_bleShutdown = true;

  if (g_adv) g_adv->stop();
  BLEDevice::deinit(true);

  // Everything below was owned by the BLE stack and has just been freed —
  // null it so nothing (e.g. setStatus/sendList) can dereference dangling
  // pointers later.
  cmdChar = nullptr;
  dataChar = nullptr;
  statusChar = nullptr;
  ...
  // Any transfer in flight is meaningless without a peer.
  uploading = false;
...
class ServerCB : public BLEServerCallbacks {
  ...
  void onDisconnect(BLEServer* pServer) override {
    ...
    if (uploadFile) uploadFile.close();
    delay(300);
    // Give the user another full window to reconnect before powering down.
    g_bleWindowStartMs = millis();
...
void loop() {
  ...
  if (!g_bleShutdown && !g_connected) {
    // Idle too long with nobody paired -> power the radio down for good.
    if (millis() - g_bleWindowStartMs > BLE_IDLE_TIMEOUT_MS) {
      shutdownBle();
    } else {
